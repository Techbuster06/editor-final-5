...
function updateFloatingControls(node) {
    ...
    if (node && node.getAttr('isMedia')) {
        ...
        const tabButtons = document.querySelectorAll('.right-sidebar-tabs .right-tab-button');
    const tabContents = document.querySelectorAll('.right-sidebar-content-container .right-tab-content');
    ...
    if (node && node.className !== 'Stage') {
        // An element is selected, activate the 'Style' tab
        targetId = 'style-props';
    } else {
        // Nothing is selected, activate the 'Elements' tab
        targetId = 'emoji-props';
    }

    const targetButton = document.querySelector(`.right-tab-button[data-target="${targetId}"]`);
    ...
    function setupEventListeners() {
        ...
        if (opacitySlider) {
            opacitySlider.addEventListener('input', function() {
                if (selectedNode) {
                    selectedNode.opacity(parseFloat(this.value) / 100);
                    setupSidebar(selectedNode);
                    layer.batchDraw();
                }
            });
        ...
        if (fontFamilySelect) {
            fontFamilySelect.addEventListener('change', function() {
                if (selectedNode && selectedNode.getClassName() === 'Text') {
                    selectedNode.fontFamily(this.value);
                    layer.batchDraw();
                    saveState();
                }
            });
...
if (sWidthSlider) {
    ...
    sWidthSlider.addEventListener('change', saveState);
            ...
            shadowToggle.addEventListener('change', function() {
                if (selectedNode) {
                    if (this.checked) {
                        ...
                        selectedNode.shadowColor('black');
                        selectedNode.shadowBlur(10);
                        selectedNode.shadowOffset({ x: 5, y: 5 });
                        selectedNode.shadowOpacity(0.5);
                    } else {
                    ...
                    if (selectedNode.getClassName() === 'Image') selectedNode.cache();
                    layer.batchDraw();
                    saveState();
                } else {
                    this.checked = false;
        ...
        if (shadowColor) shadowColor.addEventListener('input', function() {
            if (selectedNode) {
                selectedNode.shadowColor(this.value);
                layer.draw();
                saveState();
            }
        });
        ...
        if (shadowBlur) shadowBlur.addEventListener('input', function() {
            if (selectedNode) {
                selectedNode.shadowBlur(parseFloat(this.value));
                layer.draw();
                saveState();
            }
        });
        ...
        if (shadowOffsetX) shadowOffsetX.addEventListener('input', function() {
            if (selectedNode) {
                selectedNode.shadowOffsetX(parseFloat(this.value));
                layer.draw();
                saveState();
            }
        });
        ...
        if (shadowOffsetY) shadowOffsetY.addEventListener('input', function() {
            if (selectedNode) {
                selectedNode.shadowOffsetY(parseFloat(this.value));
                layer.draw();
                saveState();
            }
        });
        ...
        if (animationSelect) {
            animationSelect.addEventListener('change', function() {
                if (selectedNode) {
                    applyAnimation(selectedNode, this.value);
                    saveState();
                }
            });
        }
        ...
        if (container) {
            ...
            container.addEventListener('dragleave', function (e) {
                if (mockup) mockup.style.boxShadow = '0 0 0 5px #000, 0 10px 30px rgba(0, 0, 0,.5)';
            });
